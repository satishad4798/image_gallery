...
if (isset($_POST['innerimg'])) {
	...
	echo "inside innerimg         ";

	for ($i = 0; $i < count($_FILES['img']['name']); $i++) {
		// echo $_FILES['img']['name'][$i];
		$coverID = $_POST['cover_id'];
		$path = '../images/' . $_SESSION['albumName'] . '/';
		$path2 = '../images/' . $_SESSION['albumName'];
		...
		if (isset($_FILES['img'])) {
			...
			$imgName =  $_SESSION['albumName'] . '/' . $_FILES['img']['name'][$i];
			$imgFinalPath = $path . $_FILES['img']['name'][$i];
			echo "</br>";
			echo  "   img+fin_path" . $imgFinalPath;
